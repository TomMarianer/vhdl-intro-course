...
signal NEXT_COUNTER_VALUE   : std_logic_vector(4 downto 0);
...
begin
    ...
    STATE_MACHINE_TRANSITIONS: process (CURRENT_STATE, CURRENT_TEMP_REG, DESIRED_TEMP_REG, COOL_REG, AC_READY_REG, HEAT_REG, FURNACE_HOT_REG, COUNTER_VALUE)
    begin
        ...
        case CURRENT_STATE is
            ...
            when AC_DONE =>
                if (AC_READY_REG = '0') and (unsigned(COUNTER_VALUE) = 0) then
                    NEXT_STATE <= IDLE;
                end if;
            ...
            when FURNACE_DONE =>
                if (FURNACE_HOT_REG ='0') and (unsigned(COUNTER_VALUE) = 0) then
                    NEXT_STATE <= IDLE;
                end if;
    ...
    COUNTER_VALUE_PROCESS: process(CLK, RESET)
    begin
        if RESET = '1' then
    ...
    NEXT_COUNTER_VALUE_PROCESS: process(COUNTER_VALUE, CURRENT_STATE, NEXT_STATE)
    begin
        if (CURRENT_STATE = IDLE) and (NEXT_STATE = IDLE) then
            NEXT_COUNTER_VALUE <= (others => '0');

        elsif (CURRENT_STATE = AC_NOW_READY) and (NEXT_STATE = AC_DONE) then
            NEXT_COUNTER_VALUE <= std_logic_vector(to_unsigned(20, COUNTER_VALUE'length));

        elsif (CURRENT_STATE = FURNACE_NOW_READY) and (NEXT_STATE = FURNACE_DONE) then
            NEXT_COUNTER_VALUE <= std_logic_vector(to_unsigned(10, COUNTER_VALUE'length));

        elsif unsigned(COUNTER_VALUE) > 0 then
            NEXT_COUNTER_VALUE <= std_logic_vector(unsigned(COUNTER_VALUE) - 1);
